...
RUN --mount=type=secret,id=NEXT_GROQ_API_KEY \
    ...
    --mount=type=secret,id=NOTICIAS_API_URL \
    --mount=type=secret,id=NEXT_PUBLIC_COWORKING_URL \
    ...
    NOTICIAS_API_URL=$(cat /run/secrets/NOTICIAS_API_URL) \
    NEXT_PUBLIC_COWORKING_URL=$(cat /run/secrets/NEXT_PUBLIC_COWORKING_URL) \
